...
const AIInsights: React.FC<AIInsightsProps> = ({ analysis, hybridRisk, digitalTwin, isLoading, onGenerate }) => {
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 min-h-[400px] flex flex-col stat-card print:border-none print:p-0 print:shadow-none">
      ...
      <div className="hidden print:block mb-4 border-b border-gray-300 pb-2">
         ...
         <p className="text-sm text-gray-500 mt-1">Integrated Multi-Model Analysis (Random Forest + Logistic Regression + Local Heuristics)</p>
      </div>

      {!analysis && !isLoading && (
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 text-sm text-center print-hidden">
          <Bot className="w-12 h-12 mb-3 opacity-20" />
          <p>Click "Generate Report" or "Download Report" to analyze water spread, volume anomalies, and predict risks using local models.</p>
        </div>
      )}
      ...
      {analysis && !isLoading && (
        <div className="animate-fade-in space-y-4">
          ...
          {hybridRisk && (
            <div className="bg-slate-950/50 rounded-lg p-4 border border-slate-800 space-y-4 print:bg-white print:border-gray-200 print:p-4 print:shadow-sm print:break-inside-avoid">
               <h4 className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2 print:text-gray-700 print:text-sm print:mb-4 border-b border-slate-800 pb-2 print:border-gray-300">
                  <Activity size={14} className="text-purple-400 print:text-purple-600" />
                  Hybrid AI Engine (CatBoost + EIF)
               </h4>
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   <p className="text-xs text-slate-400">Flood Risk</p>
                   <p className="text-lg font-bold text-cyan-400">{(hybridRisk.hybrid_flood_risk * 100).toFixed(1)}%</p>
                 </div>
                 <div>
                   <p className="text-xs text-slate-400">Drought Risk</p>
                   <p className="text-lg font-bold text-orange-400">{(hybridRisk.hybrid_drought_risk * 100).toFixed(1)}%</p>
                 </div>
                 <div>
                   <p className="text-xs text-slate-400">EIF Anomaly Score</p>
                   <p className="text-lg font-bold text-red-400">{hybridRisk.eif_anomaly.anomaly_score.toFixed(2)}</p>
                 </div>
                 <div>
                   <p className="text-xs text-slate-400">CatBoost Flood Prob</p>
                   <p className="text-lg font-bold text-blue-400">{(hybridRisk.catboost_probs.flood_prob * 100).toFixed(1)}%</p>
                 </div>
               </div>
            </div>
          )}
          ...
          {digitalTwin && (
            <div className="bg-slate-950/50 rounded-lg p-4 border border-slate-800 space-y-4 print:bg-white print:border-gray-200 print:p-4 print:shadow-sm print:break-inside-avoid">
               <h4 className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2 print:text-gray-700 print:text-sm print:mb-4 border-b border-slate-800 pb-2 print:border-gray-300">
                  <Activity size={14} className="text-emerald-400 print:text-emerald-600" />
                  Digital Twin Simulation
               </h4>
               <div className="space-y-2">
                 <p className="text-xs text-slate-400">Projected Volumes (Next 3 Days)</p>
                 <div className="flex gap-2">
                   {digitalTwin.simulated_volumes.map((v: number, i: number) => (
                     <div key={i} className="flex-1 bg-slate-900 p-2 rounded text-center border border-slate-800">
                       <p className="text-[10px] text-slate-500">Day {i+1}</p>
                       <p className="font-mono text-sm text-slate-200">{v.toFixed(1)}</p>
                     </div>
                   ))}
                 </div>
                 <div className="flex gap-4 mt-2">
                   {digitalTwin.overflow_risk && <span className="text-xs text-red-400 flex items-center gap-1"><AlertTriangle size={12}/> Overflow Warning</span>}
                   {digitalTwin.drought_risk && <span className="text-xs text-orange-400 flex items-center gap-1"><AlertTriangle size={12}/> Drought Warning</span>}
                 </div>
               </div>
            </div>
          )}
