...
import { SimulationState, SeasonalData, LakeSummaryEntry } from '../types';
    ...
    <div className="flex flex-col gap-4 mb-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-stretch">
        ...
        <div className="md:w-1/3 bg-slate-800/50 border border-slate-700 p-3 rounded-lg flex flex-col justify-center">
          ...
          {/* Live status strip for all reservoirs */}
          {lakeSummaries && lakeSummaries.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {lakeSummaries.map((ls) => {
                const alert = (ls.latest_alert ?? 'NORMAL').toUpperCase();
                const dotColor = ALERT_DOT[alert] || ALERT_DOT.NORMAL;
                const isSelected = ls.id === state.selectedReservoirId;
                return (
                  <button
                    key={ls.id}
                    onClick={() => onChange({ selectedReservoirId: ls.id })}
                    title={`${ls.name} — ${alert}`}
                  ...
                  >
                    ...
                    <span className="truncate max-w-[80px]">{ls.name.split(' ')[0]}</span>
                  </button>
                );
              })}
            </div>
